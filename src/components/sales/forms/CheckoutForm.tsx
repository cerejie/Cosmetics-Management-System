import { App, AutoComplete, Button, Collapse, Form, Input, InputNumber, Select } from 'antd';
import { CreditCardOutlined, DeleteOutlined, ShoppingCartOutlined, UserOutlined } from '@ant-design/icons';
  ...
  return (
    ...
    >
      <Form.Item name="customerName" label="Customer" rules={[...rules.customerName]}>
        ...
        >
          <Input prefix={<UserOutlined style={{ color: '#94a3b8' }} />} placeholder="Walk-in customer" />
        </AutoComplete>
      </Form.Item>
        ...
        items={[
          {
            ...
            children: (
              <>
                <Form.Item label="Discount" required={false} style={{ marginBottom: 20 }}>
      ...
      <div className={styles.summary}>
        <div className={styles.row}>
          <span>Items</span>
          <span className={styles.rowValue}>{itemCount}</span>
        </div>
      ...
      <div className={styles.actions}>
        ...
        <Button block danger size="large" icon={<DeleteOutlined />} onClick={clearCart}>
          Clear cart
          <ShortcutHint keyLabel="F3" />
        </Button>
      </div>
    </Form>
  );
